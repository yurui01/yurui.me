      ...
      <div className="border-l border-zinc-200 px-4 mt-12">
        {allPosts.map((post: Post) => (
          <div key={post._id} className="mt-8 flex items-start">
            ...
            <a href={`/posts/${post._raw.flattenedPath}`} className="rounded-xl p-4 hover:bg-zinc-50 max-w-xl w-full">
              <div className="flex flex-col">
                <h2 className="font-semibold">{post.title}</h2>
                <p className="text-gray-500 mt-2 text-sm tracking-wider leading-relaxed line-clamp-4">{post.summary}</p>
              </div>
            </a>
